...
class CMB2_Types {
	...
	public function render_repeatable_field() {
		...
		<p class="cmb-add-row">
			<a data-selector="<?php echo $table_id; ?>" class="cmb-add-row-button button" href="#"><?php echo esc_html( $this->_text( 'add_row_text', __( 'Add Row', 'cmb2', 'helpguru' ) ) ); ?></a>
		</p>
	...
	protected function repeat_row( $disable_remover = false, $class = 'cmb-repeat-row' ) {
		...
		<div class="cmb-row <?php echo $class; ?>">
			...
			<div class="cmb-td cmb-remove-row">
				<a class="button cmb-remove-row-button<?php echo $disabled; ?>" href="#"><?php echo esc_html( $this->_text( 'remove_row_text', __( 'Remove', 'cmb2', 'helpguru' ) ) ); ?></a>
			</div>
		</div>
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. esc_html( $this->_text( 'no_terms_text', __( 'No terms', 'cmb2', 'helpguru' ) ) ) .'</label></li>';
		} else {
			$option_none  = $this->field->args( 'show_option_none' );
			if( ! empty( $option_none ) ) {
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => esc_html( $this->_text( 'add_upload_file_text', __( 'Add or Upload File', 'cmb2', 'helpguru' ) ) ),
			'name'  => '', 'id'  => '',
		) );
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img-status">',
						...
						'<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button">'. esc_html( $this->_text( 'remove_image_text', __( 'Remove Image', 'cmb2', 'helpguru' ) ) ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						esc_html( $this->_text( 'file_text', __( 'File:', 'cmb2', 'helpguru' ) ) ) ,' <strong>', $title ,'</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl ,'" target="_blank" rel="external">', esc_html( $this->_text( 'file-download-text', __( 'Download', 'cmb2', 'helpguru' ) ) ) ,'</a> / <a href="#" class="cmb2-remove-file-button">', esc_html( $this->_text( 'remove_text', __( 'Remove', 'cmb2', 'helpguru' ) ) ) ,'</a>)
						', $id_input ,'
					</li>';
				}
			}
	...
	public function file() {
		...
		'<input class="cmb2-upload-button button" type="button" value="'. esc_attr( $this->_text( 'add_upload_file_text', __( 'Add or Upload File', 'cmb2', 'helpguru' ) ) ) .'" />',
		$this->_desc( true );
		...
		'<div id="', $this->_id( '-status' ) ,'" class="cmb2-media-status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo $image;
					echo '<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button" rel="', $cached_id, '">'. esc_html( $this->_text( 'remove_image_text', __( 'Remove Image', 'cmb2', 'helpguru' ) ) ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo esc_html( $this->_text( 'file_text', __( 'File:', 'cmb2', 'helpguru' ) ) ), ' <strong>', $title ,'</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value ,'" target="_blank" rel="external">', esc_html( $this->_text( 'file-download-text', __( 'Download', 'cmb2', 'helpguru' ) ) ) ,'</a> / <a href="#" class="cmb2-remove-file-button" rel="', $cached_id, '">', esc_html( $this->_text( 'remove_text', __( 'Remove', 'cmb2', 'helpguru' ) ) ) ,'</a>)';
				}
			}
		echo '</div>';
	}
